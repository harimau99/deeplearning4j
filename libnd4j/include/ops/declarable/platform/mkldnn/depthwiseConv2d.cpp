...
static void depthwiseConv2dMKLDNN(const NDArray* input, const NDArray* weights, const NDArray* bias, NDArray* output,
    ...
    // input [bS, iC, iH, iW] nchw, mkl doesn't support nhwc format
    // weights [kH, kW, iC, mC], mkl doesn't support this format, so we shall make permute latter
    // bias [oC], may be nullptr
    // output [bS, oC, oH, oW] nchw
    // oC = iC*mC
    ...
    ConvolutionUtils::getSizesAndIndexesConv2d(true, *input, *output, bS, iC, iH, iW, oC, oH, oW, indIOioC, indIiH, indWiC, indWmC, indWkH, indOoH);
    ...
    w_user_md.data.format_desc.blocking.strides[0] = weights->stridesOf()[2];   // permute
    w_user_md.data.format_desc.blocking.strides[1] = weights->stridesOf()[3];
    w_user_md.data.format_desc.blocking.strides[1] = 0;
    w_user_md.data.format_desc.blocking.strides[3] = weights->stridesOf()[0];
    w_user_md.data.format_desc.blocking.strides[4] = weights->stridesOf()[1];
...
static void depthwiseConv2dNackPropMKLDNN(const NDArray* input, const NDArray* weights, const NDArray* gradO, NDArray* gradI, NDArray* gradW, NDArray* gradB,
    ...
    // input, gradI [bS, iC, iH, iW] nchw, mkl doesn't support nhwc format
    // weights, gradW [kH, kW, iC, mC], mkl doesn't support this format, so we shall make permute latter
    // gradB [oC], may be nullptr
    // gradO [bS, oC, oH, oW] nchw
    // oC = iC*mC

    int bS, iC, iH, iW, mC, oC, oH, oW;                             // batch size, input channels, input height/width, output channels, output height/width;
    int indIOioC, indIiH, indWmC, indWiC, indWkH, indOoH;           // corresponding indexes
    ConvolutionUtils::getSizesAndIndexesConv2d(true, *input, *gradO, bS, iC, iH, iW, oC, oH, oW, indIOioC, indIiH, indWiC, indWmC, indWkH, indOoH);
    ...
    gradW_user_md.data.format_desc.blocking.strides[0] = gradW->stridesOf()[2];   // permute
    gradW_user_md.data.format_desc.blocking.strides[1] = gradW->stridesOf()[3];
    gradW_user_md.data.format_desc.blocking.strides[1] = 0;
    gradW_user_md.data.format_desc.blocking.strides[3] = gradW->stridesOf()[0];
    gradW_user_md.data.format_desc.blocking.strides[4] = gradW->stridesOf()[1];
...
PLATFORM_IMPL(depthwise_conv2d) {
    ...
    depthwiseConv2dMKLDNN(input, weights, bias, output, kH, kW, sH, sW, pH, pW, dH, dW, paddingMode);
...
PLATFORM_IMPL(depthwise_conv2d_bp) {
    ...
    depthwiseConv2dNackPropMKLDNN(input, weights, gradO, gradI, gradW, gradB, kH, kW, sH, sW, pH, pW, dH, dW, paddingMode);
